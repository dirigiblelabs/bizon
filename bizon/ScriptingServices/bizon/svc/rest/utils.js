...
"use strict";
	...
	exports.asRestAPI = function(dao) {
		...
		var printError = function(httpCode, errCode, errMessage, errContext, contentType) {
		...
		var create = function(context){
		    ...
		    try{
				entity[dao.getPrimaryKey()] = dao.insert(entity, context.queryParams.cascaded);
	        	...
	        	printError(errorCode, errorCode, e.message, e.errContext);
	        	throw e;
			}		
		};
		...
		var get = function(context){
		    ...
		    try{
				...
				if(!item){
	        		...
	        		printError(response.NOT_FOUND, 1, "Record with id: " + id + " does not exist.");
	        		return;
				}
		...
		var validateQueryInputs = function(context){
			...
			if (offset === undefined || offset === null) {
				...
				printError(response.BAD_REQUEST, 1, "Invallid offset parameter: " + offset + ". Must be a positive integer.");
				context.err = {
			...
			if (limit === undefined || limit === null) {
				...
				printError(response.BAD_REQUEST, 1, "Invallid limit parameter: " + limit + ". Must be a positive integer.");
				context.err = {
					httpCode: response.BAD_REQUEST, 
					errCode: 1, 
					message: "Invallid offset parameter: " + offset + ". Must be a positive integer."
			...
			if( sort !== null && this.validSortPropertyNames && this.validSortPropertyNames.indexOf(sort)<0){
				printError(response.BAD_REQUEST, 1, "Invalid sort by property name: " + sort);
				context.err = {
					httpCode: response.BAD_REQUEST, 
					errCode: 1, 
			...
			if(order!==null){
				if(sort === null){
					printError(response.BAD_REQUEST, 1, "Parameter order is invalid without paramter sort to order by.");
					context.err = {
						httpCode: response.BAD_REQUEST, 
						errCode: 1, 
					...
					printError(response.BAD_REQUEST, 1, "Invallid order parameter: " + order + ". Must be either ASC or DESC.");
					context.err = {
						httpCode: response.BAD_REQUEST, 
						errCode: 1, 
	...
	/*	    	response.println(itemsCount);      	 */
		...
		function resourceHandlerLocator(requestPath, method, queryParams, cfg){	
			...
			if(matches[0]){
				...
				if(verbHandler){
					var acceptsMediaType = request.getHeader('Accept');
					if(acceptsMediaType){
					...
					if((acceptsMediaType && matchedProducedMIME) || (contentType && matchedConsumesMIME) || (!acceptsMediaType && !verbHandler.produces) || (!contentType && !verbHandler.consumes)
						...
						if(verbHandler.beforeHandle && typeof verbHandler.beforeHandle === 'function')
							verbHandler.beforeHandle.apply(self, [ctx]);
						if(!ctx.err)
							verbHandler.handler.apply(self, [ctx]);
					}
				}
			}
	  	...
	  	this.service = function(){
			...
			console.info('[REST API] Servicing HTTP Verb['+ method.toUpperCase() +'] for resource[/'+path+'] with query parameters: ' + queryParams);
			resourceHandlerLocator.apply(self, [path, method, queryParams, self.cfg]);	  		
	  	};
